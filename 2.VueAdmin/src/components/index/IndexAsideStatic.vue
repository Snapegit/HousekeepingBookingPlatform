...
export default {
	...
	mounted() {
		...
		this.$http({
			...
			if (data && data.code === 0) {
				if(sessionTable == 'jiazhengrenyuan') {
					this.avatar = data.data.touxiang
				}
				if(sessionTable == 'fangzhu') {
					this.avatar = data.data.touxiang
				}
				if(sessionTable == 'jiazhenggongsi') {
					this.avatar = data.data.fengmian
				}
