	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.paidanbianhao" label="派单编号" prop="paidanbianhao">
					<el-input v-model="ruleForm.paidanbianhao" placeholder="派单编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="派单时间" prop="paidanshijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.paidanshijian" 
						type="datetime"
						:readonly="ro.paidanshijian"
						placeholder="派单时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.paidanshijian" label="派单时间" prop="paidanshijian">
					<el-input v-model="ruleForm.paidanshijian" placeholder="派单时间" readonly></el-input>
				</el-form-item>
			</template>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `fuwupaidan/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.fuwupaidanCrossAddOrUpdateFlag = false;
											this.parent.search();
