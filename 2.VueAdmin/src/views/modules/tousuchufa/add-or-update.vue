	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="处罚时间" prop="chufashijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.chufashijian" label="处罚时间" prop="chufashijian">
					<el-input v-model="ruleForm.chufashijian" placeholder="处罚时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="投诉处罚" prop="tousuchufa">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="投诉处罚"
					  v-model="ruleForm.tousuchufa" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.tousuchufa" label="投诉处罚" prop="tousuchufa">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.tousuchufa}}</span>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="处罚原因" prop="chufayuanyin">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="处罚原因"
					  v-model="ruleForm.chufayuanyin" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.chufayuanyin" label="处罚原因" prop="chufayuanyin">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}'>{{ruleForm.chufayuanyin}}</span>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.chufashijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='tousuchufa'){
							this.ruleForm.tousuchufa = obj[o];
							this.ro.tousuchufa = true;
							continue;
						}
						if(o=='chufashijian'){
						...
						if(o=='chufayuanyin'){
							this.ruleForm.chufayuanyin = obj[o];
							this.ro.chufayuanyin = true;
							continue;
						}
				}
			}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `tousuchufa/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.tousuchufaCrossAddOrUpdateFlag = false;
											this.parent.search();
