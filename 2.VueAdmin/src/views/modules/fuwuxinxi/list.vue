<template>
	<div class="main-content" :style='{"width":"calc(100% - 60px)","padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","background":"#fff"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('fuwuxinxi','新增')" type="success" @click="addOrUpdateHandler()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fuwuxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('fuwuxinxi','派单')" @click="fuwupaidanCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="del" v-if="isAuth('fuwuxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<fuwupaidan-cross-add-or-update v-if="fuwupaidanCrossAddOrUpdateFlag" :parent="this" ref="fuwupaidanCrossaddOrUpdate"></fuwupaidan-cross-add-or-update>
...
import fuwupaidanCrossAddOrUpdate from "../fuwupaidan/add-or-update";
	export default {
		data() {
			return {
				fuwuleixingOptions: [],
				...
				fuwupaidanCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			fuwupaidanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.fuwupaidanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','fuwuxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.fuwupaidanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.fuwupaidanCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	fuwuxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'fuwuxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
