<template>
	<div class="main-content" :style='{"width":"calc(100% - 60px)","padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","background":"#fff"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#9E9E9E","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">服务名称</label>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					<el-button class="add" v-if="isAuth('jiedanfankui','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia7" :style='{"margin":"0 2px","fontSize":"14px","color":"#0E9BD3","height":"40px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jiedanfankui','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu7" :style='{"margin":"0 2px","fontSize":"14px","color":"#0E9BD3","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('jiedanfankui','支付')" :disabled="dataListSelections.length?false:true" type="success" @click="payBatch()">
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false'  
						prop="jiazhengzhanghao"
						label="家政账号">
						<template slot-scope="scope">
							{{scope.row.jiazhengzhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='false'  
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('jiedanfankui','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jiedanfankui','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan18" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiedanfankui','评价房主')" @click="pingjiafangzhuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								评价房主
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiedanfankui','评价家政人员')" @click="pingjiajiazhengrenyuanCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								评价家政人员
							</el-button>
							<el-button class="btn8" v-if="isAuth('jiedanfankui','投诉反馈')" @click="tousufankuiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-qita19" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								投诉反馈
							</el-button>
							<el-button class="edit" v-if=" isAuth('jiedanfankui','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('jiedanfankui','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<pingjiafangzhu-cross-add-or-update v-if="pingjiafangzhuCrossAddOrUpdateFlag" :parent="this" ref="pingjiafangzhuCrossaddOrUpdate"></pingjiafangzhu-cross-add-or-update>
		<pingjiajiazhengrenyuan-cross-add-or-update v-if="pingjiajiazhengrenyuanCrossAddOrUpdateFlag" :parent="this" ref="pingjiajiazhengrenyuanCrossaddOrUpdate"></pingjiajiazhengrenyuan-cross-add-or-update>
		<tousufankui-cross-add-or-update v-if="tousufankuiCrossAddOrUpdateFlag" :parent="this" ref="tousufankuiCrossaddOrUpdate"></tousufankui-cross-add-or-update>
...
import pingjiafangzhuCrossAddOrUpdate from "../pingjiafangzhu/add-or-update";
import pingjiajiazhengrenyuanCrossAddOrUpdate from "../pingjiajiazhengrenyuan/add-or-update";
import tousufankuiCrossAddOrUpdate from "../tousufankui/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				pingjiafangzhuCrossAddOrUpdateFlag: false,
				pingjiajiazhengrenyuanCrossAddOrUpdateFlag: false,
				tousufankuiCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			pingjiafangzhuCrossAddOrUpdate,
			pingjiajiazhengrenyuanCrossAddOrUpdate,
			tousufankuiCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			pingjiafangzhuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.pingjiafangzhuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiedanfankui');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.pingjiafangzhuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.pingjiafangzhuCrossaddOrUpdate.init(row.id,type);
				});
			},
			pingjiajiazhengrenyuanCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.pingjiajiazhengrenyuanCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiedanfankui');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.pingjiajiazhengrenyuanCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.pingjiajiazhengrenyuanCrossaddOrUpdate.init(row.id,type);
				});
			},
			tousufankuiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				if(crossOptPay=='是'&&row.ispay!='已支付'){
					this.$message({
						message: "请支付完成再操作",
						type: "success",
				...
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.tousufankuiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiedanfankui');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.tousufankuiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.tousufankuiCrossaddOrUpdate.init(row.id,type);
				});
			},
			payHandler(row){
				this.$storage.set('paytable','jiedanfankui');
	...
	jiedanfankuistatusChange(e,row){
		if(row.status==0){
