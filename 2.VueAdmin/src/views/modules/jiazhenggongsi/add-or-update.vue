	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0 30px","background":"#fff"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="服务电话" prop="fuwudianhua">
					<el-input v-model="ruleForm.fuwudianhua" placeholder="服务电话" clearable  :readonly="ro.fuwudianhua"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="服务电话" prop="fuwudianhua">
					<el-input v-model="ruleForm.fuwudianhua" placeholder="服务电话" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="注册时间" prop="zhuceshijian">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.zhuceshijian" 
						type="date"
						:readonly="ro.zhuceshijian"
						placeholder="注册时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.zhuceshijian" label="注册时间" prop="zhuceshijian">
					<el-input v-model="ruleForm.zhuceshijian" placeholder="注册时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="公司介绍" prop="gongsijieshao">
					<editor 
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.gongsijieshao" label="公司介绍" prop="gongsijieshao">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#000","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.gongsijieshao"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.gongsijieshao = this.ruleForm.gongsijieshao.replace(reg,'../../../springbootx1f0vrd9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `jiazhenggongsi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiazhenggongsiCrossAddOrUpdateFlag = false;
											this.parent.search();
