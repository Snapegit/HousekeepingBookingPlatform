...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="公司名称" prop="gongsimingcheng">
            <el-select  @change="gongsimingchengChange" v-model="ruleForm.gongsimingcheng" placeholder="请选择公司名称">
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="申请时间" prop="shenqingshijian">
              <el-date-picker
				  :disabled="true  ||ro.shenqingshijian"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="签约说明" prop="qianyueshuoming">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="签约说明"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="签约材料" prop="qianyuecailiao">
            <editor 
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/jiazhenggongsi/gongsimingcheng', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      gongsimingchengChange () {
        this.$http.get('follow/jiazhenggongsi/gongsimingcheng?columnValue=' + this.ruleForm.gongsimingcheng, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.gongsidizhi){
              this.ruleForm.gongsidizhi = res.data.data.gongsidizhi
            }
            if(res.data.data.fuzeren){
              this.ruleForm.fuzeren = res.data.data.fuzeren
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jiazhengqianyue/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jiazhengqianyue/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jiazhengqianyue/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
