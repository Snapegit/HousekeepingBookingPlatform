...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="派单编号" prop="paidanbianhao">
            <el-input v-model="ruleForm.paidanbianhao" 
                placeholder="派单编号" clearable :disabled=" false  ||ro.paidanbianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="处罚时间" prop="chufashijian">
              <el-date-picker
				  :disabled=" false  ||ro.chufashijian"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="投诉处罚" prop="tousuchufa">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="投诉处罚"
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="处罚原因" prop="chufayuanyin">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="处罚原因"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`tousuchufa/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('tousuchufa/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`tousuchufa/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
