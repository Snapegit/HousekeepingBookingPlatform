...
<div>
	...
	<div class="news-preview-pv" :style='{"minHeight":"660px","width":"100%","padding":"10px 10% 0","margin":"0px auto","position":"relative","background":"#fff"}'>
		<el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 0 0","alignItems":"center","top":"0px","flexWrap":"wrap","background":"#fff","display":"flex","width":"300px","position":"absolute","right":"10%","justifyContent":"center","height":"auto"}'>
			<el-form-item :style='{"margin":"0 0px"}'>
				<el-input v-model="title" placeholder="标题"></el-input>
			</el-form-item>
			<el-button type="primary" @click="getNewsList(1)" :style='{"cursor":"pointer","border":"0","padding":"0px 20px 0 10px","margin":"0 0px 0 10px","outline":"none","color":"#fff","borderRadius":"0 0px 0px 0","background":"#ff777f","width":"80px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>
		...
		<!-- category -->
		<div class="category" :style='{"padding":"30px 0 20px","margin":"60px 0 0","borderColor":"#ddd","flexWrap":"wrap","background":"#fff","borderWidth":"0px 0","display":"flex","width":"300px","borderStyle":"solid","float":"right","justifyContent":"flex-start","height":"auto"}'>
		  ...
		  <div v-for="(item,index) in categoryList" @click="categoryClick(index+1)" :key="index" class="item" :class="categoryIndex == index+1 ? 'active' : ''">{{item.typename}}</div>
				...
				<!-- 样式五 -->
		<div class="list5 index-pv1" :style='{"minHeight":"660px","padding":"0","margin":"20px 0","color":"#a6a6a6","flexWrap":"wrap","background":"#fff","display":"block","width":"calc(100% - 340px)","fontSize":"12px","float":"left","height":"auto"}'>
			<div :style='{"cursor":"pointer","padding":"0 0 20px","margin":"0 0 20px","borderColor":"#eee","background":"#fff","borderWidth":"0 0 1px","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' v-for="item in newsList" :key="item.id" @click="toNewsDetail(item)" class="list-item animation-box">
				<el-image :style='{"border":"0px solid #ddd","padding":"0px","margin":"0 20px 0 0","objectFit":"cover","display":"inline-block","width":"200px","height":"120px"}' :src="baseUrl + item.picture" :fit="fill"></el-image>
				<div class="item-info" :style='{"padding":"0px","alignItems":"flex-start","flexWrap":"wrap","flex":"1","display":"block","width":"calc(100% - 320px)","position":"relative","height":"120px"}'>
					<div :style='{"overflow":"hidden","whiteSpace":"nowrap","color":"#555","width":"100%","lineHeight":"32px","fontSize":"16px","textOverflow":"ellipsis","fontWeight":"600","height":"32px"}' class="title ">{{item.title}}</div>
					<div :style='{"margin":"0 0 10px","overflow":"hidden","color":"#666","width":"100%","lineHeight":"30px","fontSize":"14px","order":"2","height":"60px"}' class="introduction ">{{item.introduction}}</div>
		...
		<!-- 热门信息 -->
		<div class="hot" :style='{"border":"0px solid #e9e9e9","padding":"0","margin":"0","background":"none","width":"100%","clear":"both","height":"auto"}'>
		  <div :style='{"padding":"0px 20px","borderColor":"#fb3f52","color":"#fb3f52","textAlign":"left","display":"inline-block","borderRadius":"0px","background":"url() no-repeat left bottom","borderWidth":"0 0 4px","width":"auto","lineHeight":"43px","fontSize":"22px","borderStyle":"solid","fontWeight":"600"}'>推荐信息</div>
		  <div :style='{"padding":"0 10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}'>
		    <div v-for="item in hotList" :key="item.id" :style='{"cursor":"pointer","width":"100%","padding":"0","margin":"0 0 0px","background":"#fff","height":"auto"}' @click="toNewsDetail(item)">
		...
		<!-- 最新动态 -->
		<div class="news" :style='{"padding":"0 7% 0","margin":"0px 0 0","textAlign":"center","background":"#fff","display":"none","width":"100%","clear":"both","height":"auto"}'>
...
<style rel="stylesheet/scss" lang="scss" scoped>
	...
	.index-pv1 .animation-box:hover {
				transform: rotate(0deg) scale(1) skew(0deg, 0deg) translate3d(12px, 0px, 0px);
