    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="家政账号" prop="jiazhengzhanghao">
            <el-input v-model="ruleForm.jiazhengzhanghao" 
                placeholder="家政账号" clearable :disabled=" false  ||ro.jiazhengzhanghao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="服务类型" prop="fuwuleixing">
            <el-select v-model="ruleForm.fuwuleixing" placeholder="请选择服务类型" :disabled=" false  ||ro.fuwuleixing" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="等级" prop="dengji">
            <el-select v-model="ruleForm.dengji" placeholder="请选择等级" :disabled=" false  ||ro.dengji" >
              <el-option
  ...
  export default {
    data() {
      ...
      return {
        ...
        rules: {
          ...
          jiazhengzhanghao: [
            { required: true, message: '家政账号不能为空', trigger: 'blur' },
          ],
          mima: [
            { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          jiazhengxingming: [
            { required: true, message: '家政姓名不能为空', trigger: 'blur' },
          ],
          touxiang: [
          ],
          xingbie: [
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/fuwuleixing/fuwuleixing', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.fuwuleixingOptions = res.data.data;
          }
        });
        this.dengjiOptions = "金牌,银牌,铜牌".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jiazhengrenyuan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jiazhengrenyuan/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jiazhengrenyuan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
