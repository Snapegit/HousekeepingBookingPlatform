...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务内容" prop="fuwuneirong">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.fangzhuxingming!=''&&json.fangzhuxingming) || json.fangzhuxingming==0){
                this.ruleForm.fangzhuxingming = json.fangzhuxingming;
				this.ro.fangzhuxingming = true;
            }
            if((json.dizhi!=''&&json.dizhi) || json.dizhi==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`fuwuxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('fuwuxinxi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`fuwuxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
