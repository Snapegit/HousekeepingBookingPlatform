...
<div :style='{"width":"100%","padding":"30px 10% 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="评价时间" prop="pingjiashijian">
              <el-date-picker
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="评价内容" prop="pingjianeirong">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="评价内容"
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.jiazhengzhanghao!=''&&json.jiazhengzhanghao) || json.jiazhengzhanghao==0){
                this.ruleForm.jiazhengzhanghao = json.jiazhengzhanghao;
				this.ro.jiazhengzhanghao = true;
            }
            if((json.jiazhengxingming!=''&&json.jiazhengxingming) || json.jiazhengxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`pingjiafangzhu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('pingjiafangzhu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`pingjiafangzhu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
