...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>家政姓名：</div>
        <el-input v-model="formSearch.jiazhengxingming" placeholder="家政姓名" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('xinzixinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 10px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"0px","background":"#c5c5c5","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/xinzixinxiAdd')"><i v-if="false" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		    ...
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","flex":"1"}'>
		        ...
		        <div :style='{"padding":"2px 0px","borderColor":"#f0f0f0","margin":"0 10px 0 0","borderStyle":"dashed","borderWidth":"0 0 0px 0","display":"inline-block"}' class="publisher">
		          ...
		          <span class="text">{{item.gongsizhanghao}}</span>
		        </div>
		      </div>
		    </div>
		  </div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'工资月份',list:this.gongziyuefenOptions,tableName: 'gongziyuefen',check: -1})
		this.$http.get('option/jiazhengrenyuan/jiazhengzhanghao').then(res => {
			if (res.data.code == 0) {
				this.jiazhengzhanghaoOptions = res.data.data;
				this.selectOptionsList.push({name:'家政账号',list:this.jiazhengzhanghaoOptions,tableName: 'jiazhengzhanghao',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.gongziyuefen != '') searchWhere.gongziyuefen = this.formSearch.gongziyuefen;
        if (this.formSearch.jiazhengzhanghao != '') searchWhere.jiazhengzhanghao = this.formSearch.jiazhengzhanghao;
        if (this.formSearch.jiazhengxingming != '') searchWhere.jiazhengxingming = '%' + this.formSearch.jiazhengxingming + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`xinzixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/xinzixinxiDetail', query: params});
      },
